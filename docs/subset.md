...
- `&matmul A B` — f32 matrix multiply for rank-2 `A [M,K]`, `B [K,N]`,
  placed on the **SME engine**.  The first slice enters and leaves streaming
  mode (proved by the `sme` engine-entry counter when SME is exposed), then
  computes the product; the ZA-accumulating tile kernel is the next slice.
